...
if Settings.Fine.Toggle then
    RegisterCommand(Settings.Fine.Command, function(source, args, rawCommand)
      TriggerEvent("ND:fines", source, args, rawCommand)
    end, false)
end
...
RegisterNetEvent("ND:fines")
AddEventHandler("ND:fines", function(source, args, rawCommand)
  local player = source
  local amount = tonumber(args[2])
  local reason = table.concat(args, " ", 3)
  local players = NDCore.Functions.GetPlayers()
  local playerCName = players[player].firstName .. ' ' .. players[player].lastName
  if players[player].job ~=  "SACO" and players[player].job ~= "MECH" then
